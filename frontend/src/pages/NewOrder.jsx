...
const NewOrder = () => {
    ...
    const generatePDF = () => {
        ...
        doc.setFontSize(16);
        doc.text("LAVANDERIA SANTI", 40, 15, { align: "center" });

        doc.setFontSize(10);
        doc.text("--------------------------------", 40, 22, { align: "center" });
        doc.text(`TICKET: ${orderRecord.ticket_number}`, 40, 28, { align: "center" });
        doc.text("--------------------------------", 40, 34, { align: "center" });

        doc.setFont("courier", "normal");
        doc.setFontSize(9);
        doc.text(`Fecha: ${new Date().toLocaleDateString()}`, 10, 45);
        doc.text(`Cliente: ${orderRecord.customer.name}`, 10, 52);
        doc.text(`Tel: ${orderRecord.customer.phone}`, 10, 59);

        doc.text("--------------------------------", 40, 68, { align: "center" });
        doc.setFont("courier", "bold");
        doc.text("DETALLE DEL SERVICIO", 40, 74, { align: "center" });
        doc.text("--------------------------------", 40, 80, { align: "center" });

        doc.setFont("courier", "normal");
        doc.text(`${orderRecord.service_type}`, 10, 90);
        doc.text(`Cant: x${orderRecord.garments_count}`, 10, 97);

        doc.setFont("courier", "bold");
        doc.setFontSize(14);
        doc.text(`TOTAL: $${orderRecord.price}`, 70, 115, { align: "right" });

        doc.setFontSize(8);
        doc.setFont("courier", "normal");
        const entrega = new Date(orderRecord.estimated_delivery).toLocaleDateString();
        doc.text("Entrega estimada: " + entrega, 10, 125);

        doc.text("--------------------------------", 40, 135, { align: "center" });
        doc.text("Gracias por su preferencia", 40, 140, { align: "center" });
        doc.text("Conserve este ticket", 40, 145, { align: "center" });
    ...
    return (
        <div className="max-w-5xl mx-auto space-y-10 animate-in fade-in duration-700 pb-20 relative">
            ...
            {showSuccessModal && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in transition-all">
                    <div className="bg-white rounded-[2.5rem] shadow-2xl max-w-lg w-full p-10 text-center space-y-8 animate-in zoom-in-95 duration-300">
                        ...
                        <div>
                            <h2 className="text-3xl font-black text-slate-900 uppercase tracking-tight">¡Pedido Exitoso!</h2>
                            <p className="text-slate-500 font-bold mt-2">El ticket <span className="text-[#ec5b13] font-black">{orderRecord?.ticket_number}</span> ha sido generado correctamente.</p>
                        </div>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <button
                                    ...
                                    className="flex items-center justify-center gap-2 bg-orange-50 text-[#ec5b13] py-4 rounded-2xl font-black uppercase tracking-wider text-xs border border-orange-100 hover:bg-orange-100 transition-all"
                                >
                                    <PlusCircle size={16} />
                                    Nuevo Pedido
                                </button>
            ...
            <div className="flex items-end justify-between">
                <div>
                    <h2 className="text-3xl font-black text-slate-900 uppercase tracking-tight">Nuevo Pedido</h2>
                    <p className="text-slate-500 font-bold mt-1">Ingresa los datos para generar un nuevo ticket de servicio.</p>
                </div>
                <div className="bg-orange-50 p-4 rounded-2xl text-[#ec5b13] shadow-sm border border-orange-100">
            ...
            <form onSubmit={handleSubmit} className="bg-white rounded-[2.5rem] shadow-xl shadow-slate-200/60 border border-slate-100 p-12 relative overflow-hidden">
                ...
                <div className="grid md:grid-cols-2 gap-16 relative">
                    <section className="space-y-10">
                        <h3 className="flex items-center gap-4 text-xl font-black text-slate-800 uppercase tracking-[0.2em] border-b border-slate-50 pb-6">
                            <span className="w-10 h-10 rounded-xl bg-slate-50 flex items-center justify-center">
                                <span className="material-symbols-outlined text-[#ec5b13]">person_add</span>
                            </span>
                            Datos del Cliente
                        </h3>
                        ...
                        <div className="space-y-8">
                            <div className="space-y-3">
                                ...
                                <div className="relative group">
                                    <span className="material-symbols-outlined absolute left-5 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-[#ec5b13] transition-colors">person</span>
                                    <input
                                        name="name"
                                        type="text"
                                        required
                                        placeholder="Ej: Juan Pérez"
                                        className="w-full pl-14 pr-6 py-5 bg-slate-50 border-2 border-transparent rounded-2xl focus:border-orange-100 focus:bg-white transition-all outline-none font-black text-slate-700 placeholder:text-slate-300"
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="relative group">
                                    <span className="material-symbols-outlined absolute left-5 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-[#ec5b13] transition-colors">phone_iphone</span>
                                    <input
                                        name="phone"
                                        type="tel"
                                        required
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="relative group">
                                    <span className="material-symbols-outlined absolute left-5 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-[#ec5b13] transition-colors">mail</span>
                                    <input
                                        name="email"
                                        type="email"
                                        required
                    ...
                    <section className="space-y-10">
                        <h3 className="flex items-center gap-4 text-xl font-black text-slate-800 uppercase tracking-[0.2em] border-b border-slate-50 pb-6">
                            <span className="w-10 h-10 rounded-xl bg-slate-50 flex items-center justify-center">
                                <span className="material-symbols-outlined text-[#ec5b13]">dry_cleaning</span>
                            </span>
                            Detalles del Pedido
                        </h3>
                        ...
                        <div className="space-y-8">
                            <div className="grid grid-cols-2 gap-6">
                                <div className="space-y-3">
                                    ...
                                    <select
                                        name="service_type"
                                        className="w-full px-6 py-5 bg-slate-50 border-2 border-transparent rounded-2xl focus:border-orange-100 focus:bg-white transition-all outline-none font-black text-slate-700 cursor-pointer appearance-none"
                                        onChange={handleChange}
                                ...
                                <div className="space-y-3">
                                    ...
                                    <input
                                        ...
                                        min="1"
                                        required
                                        className="w-full px-6 py-5 bg-slate-50 border-2 border-transparent rounded-2xl focus:border-orange-100 focus:bg-white transition-all outline-none font-black text-slate-700"
                                        onChange={handleChange}
                            ...
                            <div className="grid grid-cols-2 gap-6">
                                <div className="space-y-3">
                                    ...
                                    <div className="relative group">
                                        <span className="absolute left-6 top-1/2 -translate-y-1/2 font-black text-slate-300 group-focus-within:text-[#ec5b13] transition-colors text-xl">$</span>
                                        <input
                                            name="price"
                                            type="number"
                                            step="0.01"
                                            ...
                                            placeholder="0.00"
                                            className="w-full pl-12 pr-6 py-5 bg-slate-100 border-2 border-transparent rounded-2xl outline-none font-black text-[#ec5b13] text-2xl shadow-inner shadow-slate-100 cursor-not-allowed"
                                        />
                                    </div>
                                </div>
                                <div className="space-y-3">
                ...
                <div className="mt-16 pt-10 border-t border-slate-50 flex items-center justify-between">
                    ...
                    <div className="flex gap-4">
                        <button
                            ...
                            className="bg-[#ec5b13] hover:bg-orange-600 text-white font-black uppercase tracking-[0.2em] py-5 px-14 rounded-2xl shadow-2xl shadow-orange-500/30 transition-all hover:-translate-y-1 active:scale-95 flex items-center gap-4 text-sm disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:translate-y-0 disabled:shadow-none"
                        >
